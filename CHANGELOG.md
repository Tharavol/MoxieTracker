...
### Added
- A "Hide Concentration values under threshold" checkbox on the Thresholds options page (#48). When checked, the Concentration window skips a profession's line entirely once its value is below that profession's threshold, instead of always showing the line colored against it; a character with every profession filtered out this way is left off the list too.
